...
ServerFunc.getStatus = function(status, channel)
	...
	if status == 404 or status == 401 and webhooksFile[channel].webhook ~= "DISCORD_WEBHOOK" and webhooksFile[channel].webhook ~= "" then 
		print('^3Warn: Ykz_logs webhook. Possible invalid webhook for "'..channel..'" webhook. Status code: '..status)
	end
end
...
function GetPlayerDetails(src, config, channel)
    ...
    if GetConvar("steam_webApiKey", "false") ~= 'false' then
        ...
        _steamID = ""
        _steamURL = ""
        TriggerEvent('Pastor:Ykz_logs:errorLog', 'You need to set a steam api key in your server.cfg for the steam identifiers to work!.')
    end
    ...
    if config.Session or config.PlayTime and not webhooksFile[channel].Hide['PlayTime'] then
        if GetResourceState('Pastor_playTime') == "started" then
            playtime = exports.Pastor_playTime:getPlayTime(src)
            ...
            TriggerEvent('Pastor:Ykz_logs:errorLog', 'Pastor_playTime is not installed.')
            _total = ""
            _session = ""
        end
    else
    ...
    if args['channel'] == 'system' then
        message = {
            ...
            embeds = {{
                ...
                ["author"] = {
                    ["name"] = "Pastor",
                    ["icon_url"] = "https://pastor.com/i/DiscordIcon.png",
                    ["url"] = "https://discord.gg/pastor"
                },
                ["title"] = "📢 SYSTEM",
                ["description"] = args['description'],
                ["footer"] = {
                    ["text"] = "2020 - "..os.date("%Y").." © Pastor • "..os.date("%x %X %p"),
                    ["icon_url"] = "https://pastor.com/i/DiscordIcon.png",
                },
            }}, 
            avatar_url = "https://pastor.com/i/DiscordIcon.png"
        }
        if args['ping'] then
    ...
    if webhooksFile[args.channel] then
        ...
        if args.player_id then
            if webhooksFile[args.channel].logHistory then
                TriggerClientEvent('Pastor:ClientLogStorage', args.player_id, { Channel = args.channel, Message = args.EmbedMessage, TimeStamp = os.date("%x %X %p") })
            end
        ...
        if args.player_2_id then
            if webhooksFile[args.channel].logHistory then
                TriggerClientEvent('Pastor:ClientLogStorage', args.player_2_id, { Channel = args.channel, Message = args.EmbedMessage, TimeStamp = os.date("%x %X %p") })
            end
        ...
        print("Ykz_logs: Webhooks channel not found. ("..args.channel..")")
    end
end
